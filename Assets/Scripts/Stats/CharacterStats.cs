...
public class CharacterStats : MonoBehaviour
{
    ...
    public Stat intelligence; // 1p oint increase magic damage by 1 and magic resistance by 3
    ...
    public bool isChilled; //reduce armor bt 20%
    ...
    private static void AttemptToApplyAilments(CharacterStats _targetStats, int _fireDamage, int _iceDamage, int _lightingDamage)
    {
    ...
    public void ApplyAilments(bool _ignite, bool _chill, bool _shock)
    {
        ...
        if(_shock && canApplyShock)
        {
            if(!isShocked)
            {
                //àÍâÒñ⁄ÇÃÇ—ÇËÇ—ÇËèÛë‘Ç…Ç»Ç¡ÇƒÇ¢ÇÈÇ©Ç«Ç§Ç©
                ApplyShock(_shock);
            }
            else
            {
                ...
                HitNearrestTargetWithShockStrike();
            }
    ...
    private void HitNearrestTargetWithShockStrike()
    {
    ...
    protected virtual void DecreaseHealthBy(int _damage, bool isIgniteDamage)
    {
        if (isVulnerable)
        ...
        if(_damage > 0)
            fx.CreatePopUpText(_damage.ToString(), isIgniteDamage);
    ...
    {
        float totalCritcalPower = (critPower.GetValue() + strength.GetValue()) * .01f;

        float critDamage = _damage * totalCritcalPower;
